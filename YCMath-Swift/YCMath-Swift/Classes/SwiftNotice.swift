...
extension UIResponder {
    ...
    /// wait with text by lucien
    func pleaseWaitWithText(_ text: String) {
        SwiftNotice.waitText(text)
    }
//    func closeWaitWithText(){
//        SwiftNotice.closeWaitText()
//    }
    func pleaseWait() {
        SwiftNotice.wait()
    }
...
class SwiftNotice: NSObject {
    ...
    static var closeWaitView:UIView = UIView()
    ...
    static func waitText(_ text: String) {
        ...
        //loading
        let ai = UIActivityIndicatorView(activityIndicatorStyle: UIActivityIndicatorViewStyle.whiteLarge)
        ai.frame = CGRect(x: frame.size.width/2-18, y: 21, width: 36, height: 36)
        ai.startAnimating()
        mainView.addSubview(ai)
        //添加label
        let label = UILabel()
        label.frame = CGRect(x: 0, y: 70, width: 100, height: 20)
        label.text = text
        label.numberOfLines = 0
        label.font = UIFont.systemFont(ofSize: 13)
        label.textAlignment = NSTextAlignment.center
